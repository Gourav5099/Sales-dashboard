...
| Appendix A | Software and Tools Used | 142 |
...
```mermaid
flowchart LR
    Proj["Sales & Business Performance Analysis"]
    Proj --> SDG8["SDG 8: Decent Work & Economic Growth\n(Target 8.2: Economic Productivity via Tech Upgrade)"]
    Proj --> SDG9["SDG 9: Industry, Innovation & Infrastructure\n(Target 9.5: Enhance Scientific & Tech Capabilities)"]
    Proj --> SDG12["SDG 12: Responsible Consumption & Production\n(Target 12.6: Corporate Sustainability & Waste Reduction)"]
```
...
```mermaid
flowchart LR
    Theory["1. Theoretical Rigor\n(Lecture & Algorithmic Foundation)"] --> Lab["2. Guided Laboratory\n(Syntax, Debugging & Micro-tasks)"]
    Lab --> App["3. Capstone Application\n(End-to-End Industrial Project)"]
    App --> Def["4. Viva Defense\n(Peer Review & Oral Examination)"]
```
...
mindmap
  root((Data Quality Dimensions))
    Completeness
      Zero Nulls in Primary Keys
      Imputed Financial Values
      Inferred Categories
    Uniqueness
    ...
    Consistency
      ...
      Synchronized Cross-Table Foreign Keys
    Validity
      Quantity in Range 1 to 10
      Discount in Range 0 to 35%
...
```mermaid
erDiagram
    DIM_CALENDAR ||--o{ FACT_SALES : "filters (Order_Date -> Date)"
    DIM_PRODUCT ||--o{ FACT_SALES : "filters (Product -> Product)"
    DIM_REGION ||--o{ FACT_SALES : "filters (Region -> Region)"
...
```mermaid
flowchart TD
    Raw["Raw Data (3,000 Rows)\ndata/raw_sales_data.csv"] --> Dedup["1. Deduplication\nRemove 10 Duplicate Order_IDs"]
    Dedup --> TextClean["2. Text Standardization\nTRIM() & Title Case on Category & Region"]
    TextClean --> CatImp["3. Category Imputation\nMap Product to Catalog Category"]
    CatImp --> RegImp["4. Region Imputation\nLookup Customer Mode Region"]
    RegImp --> FinImp["5. Financial Imputation\nRe-derive Sales & Profit from Unit Economics"]
    FinImp --> Clean["Cleaned Dataset (2,990 Rows)\nsales_data_cleaning.csv"]
```
...
## Appendix B: Project Modules & Architectural Breakdown
...
## Appendix C: Complete SQL Schema and Analytical Queries (`schema_and_queries.sql`)
...
## Appendix D: Complete Power BI DAX Measures Reference Library
...
## Appendix E: Interactive Streamlit Dashboard Source Code (`app.py`)
...
## Appendix F: Automated Visualization Generator Source Code (`generate_charts.py`)
...
## Appendix G: Raw Data Quality Defects Audit Log (`DATA_QUALITY_NOTES.md`)
...
## Appendix H: Master Project Quick-Reference & Executive Compendium
